...
describe("getBalance function", () => {
    it("getBalance function exist", () => {
        assert.ok(inbox.methods.balanceOf);
    });

    it("return balance ", async () => {

        const balance = await inbox.methods.balanceOf(accounts[1]);
...
describe("getEthers function", () => {
    ...
    it("receives ethers when you sell certain amount of tokens ", async () => {
        ...
        const Before = await inbox.methods.balanceOf(accounts[1]);

        await inbox.methods.getTokens().send({
            from: accounts[1],
            value: web3.utils.toWei("90", "ether")
        });
        const after1 = await inbox.methods.balanceOf(accounts[1]);

        await inbox.methods.getEthers(900000000000000000).send({
            from: accounts[1]
        });

        const after2 = await inbox.methods.balanceOf(accounts[1]);
        const tokens = await inbox.methods.balanceOf(accounts[1]);
